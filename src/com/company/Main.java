...
import com.company.entity.factory.ProductFactory;
import com.company.entity.factory.ProductReal;
    ...
    return orders.stream()
            .collect(Collectors.toMap(order -> order,
                    order -> order.getProducts().stream()
                            .filter(product -> product instanceof ProductReal)
                            .mapToInt(product -> ((ProductReal) product).getWeight()).sum()));
  }
}
